...
// Generic extractor for any data file using img: 'ID' or img: `ID` syntax
function makeExtractor(content) {
  const ids = [];
  const re = /\bimg:\s*['`]([A-Za-z0-9]{5,12})['`]/g;
  let m;
...
const SOURCES = [
  ...
  {
    label: 'Easter eggs data (src/data/easterEggs.tsx)',
    file: path.join(ROOT, 'src', 'data', 'easterEggs.tsx'),
    extract: makeExtractor,
  },
  {
    label: 'Main quest data (src/data/mainQuest.tsx)',
    file: path.join(ROOT, 'src', 'data', 'mainQuest.tsx'),
    extract: makeExtractor,
  },
  {
